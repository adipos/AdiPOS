...
namespace Api.Repository.Config
{
    public static class Configuration
    {
        public static void Configure(IServiceCollection services, IConfiguration configuration)
        {
            ...
            var assembliesRepo = AppDomain.CurrentDomain.GetAssemblies().Where(x => x.GetName().Name == "Api.Repository")
                .FirstOrDefault().GetTypes();
            var repositoryList = assembliesRepo
                .Where(x => x.Name.EndsWith("Repository") && !x.Name.StartsWith("I"));

            foreach (var repository in repositoryList)
            {
                var nameRepository = repository.Name;
                var iRepository = assembliesRepo.Where(a => a.Name == "I" + nameRepository).FirstOrDefault();
                if (iRepository != null)
                {
